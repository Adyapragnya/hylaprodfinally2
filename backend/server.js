...
app.get('/api/sat-intervals', async (req, res) => {
  ...
  async function checkAndUpdateVesselData() {
    try {
        const aisSatPullConfig = await AisSatPull.find();
        if (!aisSatPullConfig) {
...
function sendConsolidatedEmail(vessels) {
  ...
  const mailOptions = {
      ...
      bcc: 'tech.adyapragnya@gmail.com,kadlvi@hylapps.com,abhishek,abhishek.nair@hylapps.com',
     
      subject: 'Hyla Daily Alert',
...
// Set an interval to call sendConsolidatedEmail every 4 hour (14400000 ms)
setInterval(() => {
